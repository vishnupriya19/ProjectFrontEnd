    ...
    $.fn.workflowChart = function (options) {
        ...
        return this.each(function () {
            ...
            const marker = draw.marker(10, 10, function (add) {
                add.path('M0,0 L10,5 0,10').fill('yellow').stroke({width: 2, color: settings.chartColor});
            });
            // the arrow of dashes
            ...
            $.each(nodesGroupArray, function (index, nodes) {
                ...
                $.each(nodes, function (index, node) {
                    ...
                    if (!!node.link) {
                        ...
                        // the circle
                        circle = link.circle(settings.circleSize).attr({fill: settings.chartColor, data: node.title, onclick: "displayAlert(this)" }).move(text.cx() - settings.circleSize / 2, text.cy() - settings.circleSize / 2 * 3);
                    } else {
                        ...
                        circle = draw.circle(settings.circleSize).attr({fill: settings.chartColor, data: node.title, onclick: "displayAlert(this)" }).move(text.cx() - settings.circleSize / 2, text.cy() - settings.circleSize / 2 * 3);
                        //console.log(circle);
                    }
